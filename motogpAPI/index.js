module.exports = function(app) {
	...
	app.get(BASE_PATH + '/motogp-statistics/loadInitialData', (req, res) => {
		...
		res.sendStatus(200);
		console.log(
			'Initial motogp_statistics loaded: ' + JSON.stringify(motogp_statistics, null, 2)
		);
	});
	...
	app.get(BASE_PATH + '/motogp-statistics', (req, res) => {
		...
		if (req.query.world_titleMin && req.query.world_titleMax)
			search['world_title'] = {
				$gte: parseInt(req.query.world_titleMin),
				$lte: parseInt(req.query.world_titleMax)
			};
		if (req.query.world_titleMin && !req.query.world_titleMax)
			search['world_title'] = { $gte: parseInt(req.query.world_titleMin) };
		if (!req.query.world_titleMin && req.query.world_titleMax)
			search['world_title'] = { $lte: parseInt(req.query.world_titleMax) };

		if (req.query.victoryMin && req.query.victoryMax)
			search['victory'] = {
				$gte: parseInt(req.query.victoryMin),
				$lte: parseInt(req.query.victoryMax)
			};
		if (req.query.victoryMin && !req.query.victoryMax)
			search['victory'] = { $gte: parseInt(req.query.victoryMin) };
		if (!req.query.victoryMin && req.query.victoryMax)
			search['victory'] = { $lte: parseInt(req.query.victoryMax) };

		if (req.query.podiumMin && req.query.podiumMax)
			search['podium'] = {
				$gte: parseInt(req.query.podiumMin),
				$lte: parseInt(req.query.podiumMax)
			};
		if (req.query.podiumMin && !req.query.podiumMax)
			search['podium'] = { $gte: parseInt(req.query.podiumMin) };
		if (!req.query.podiumMin && req.query.podiumMax)
			search['podium'] = { $lte: parseInt(req.query.podiumMax) };

		db
			.find(search)
			.skip(offset)
			.limit(limit)
			.exec(function(err, motogp_statistics) {
	...
	app.post(BASE_PATH + '/motogp-statistics', (req, res) => {
		...
		if (
			newStat.length == 6 ||
			!newStat.hasOwnProperty('country') ||
			!newStat.hasOwnProperty('pilot') ||
			!newStat.hasOwnProperty('last_title') ||
			!newStat.hasOwnProperty('world_title') ||
			!newStat.hasOwnProperty('victory') ||
			!newStat.hasOwnProperty('podium')
		) {
			res
				.sendStatus(400)
				.send(
					'Debe de tener los campos country, pilot, last_title, world_title, victory y podium'
				);
		} else {
			db.find({ pilot: newPilot }, (error, motogp_statistics) => {
				if (motogp_statistics != 0) {
					...
					res
						.status(400)
						.send('BAD REQUEST, Como minimo debe rellenar los campo country y pilot');
				} else {
					db.insert(newStat);
	...
	app.put(BASE_PATH + '/motogp-statistics/:pilot', (req, res) => {
		...
		if (
			!newPilot.hasOwnProperty('country') ||
			newPilot.country == null ||
			!newPilot.hasOwnProperty('pilot') ||
			newPilot.pilot == null ||
			!newPilot.hasOwnProperty('last_title') ||
			newPilot.last_title == null ||
			!newPilot.hasOwnProperty('world_title') ||
			newPilot.world_title == null ||
			!newPilot.hasOwnProperty('victory') ||
			newPilot.victory == null ||
			!newPilot.hasOwnProperty('podium') ||
			newPilot.podium == null
		) {
			res
				.status(400)
				.send(
			...
			db.find({ pilot: pilot }, function(err, motogp_statistics) {
				if (motogp_statistics == 0) {
					...
					db.update(
						...
						{
							$set: {
								country: newPilot.country,
								last_title: newPilot.last_title,
								world_title: newPilot.world_title,
								victory: newPilot.victory,
								podium: newPilot.podium
							}
						},
						{},
